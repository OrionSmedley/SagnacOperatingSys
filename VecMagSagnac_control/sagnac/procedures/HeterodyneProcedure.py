...
class voltage_sweep_procedure(Procedure):
    ...
    DATA_COLUMNS = ["ThetaK","X1","Y1","X2","Y2","DeltaThetaK","DeltaThetaK_DualSideband", "DeltaX1_C-M", "DeltaY1_C-M", "DeltaX1_C+M", "DeltaY1_C+M","TX1","TY1","TX2","TY2", "voltage", "sweep_field", "sample_temp", "vti_temp", "reservoir_temp", "elapsed_time"]
    # print("pma sweep")
    magnet = vectorMagnetFullUSB() #X,Y,Z in that order
    def startup(self):
        log.info("Connecting and configuring the instruments")
        ...
        #         self.lockin.set_vout(1,6,self.applied_voltage/10*np.sqrt(2))
        # log.info(f"set voltage to {self.lockin.get_vout(1, 6)/np.sqrt(2)}")
        # change set voltage to set magnetic field
        self.magnet.set_field_polar(self.sweep_field, self.sweep_field_azimuth, self.sweep_field_polar)
        while not self.magnet.check_field_polar(self.sweep_field, self.sweep_field_azimuth, self.sweep_field_polar, 2e-3):
            sleep(0.5)
        x,y,z = self.magnet.get_field_cartesian()
    ...
    def execute(self):
        ...
        if (self.voltage_start > self.voltage_stop):
            voltages = np.arange(self.voltage_start, 
                            self.voltage_stop, 
                            -1 * self.voltage_step)
        else: 
            voltages = np.arange(self.voltage_start, 
                            self.voltage_stop, 
                            self.voltage_step)
        if self.voltage_stop not in voltages:
            voltages = np.append(voltages,self.voltage_stop)
    ...
    def shutdown(self):
        ...
        while (abs(self.magnet.get_field_cartesian()[0]) > 0.001):
            sleep(2)
        self.magnet.shutdown()
        v_list = np.linspace(self.voltage_stop, 0,
                            100)
        for v in v_list:
            self.lockin.set_vout(1,6,v/10*np.sqrt(2))
            if np.isclose(v,0,atol=5e-5):
                break

        log.info(f"Voltage set to {0}V")