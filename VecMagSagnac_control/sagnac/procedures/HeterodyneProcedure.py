...
class sagnacHeterodyneProcedure_vm_PhiSweep(Procedure):
    ...
    applied_current = FloatParameter("Applied Sample DC Current", units="A", default=1)
    # apply_current = BooleanParameter("Current Applied?", default=True)
    # current_amplitude = FloatParameter("Applied Sample Current Amplitude", units="A", default=1)
    #current_frequency = FloatParameter("Applied Sample Current frequency", units="kHz", default=1)
    # current_offset = FloatParameter("Applied Sample Current offset", units="A", default=1)
    ...
    DATA_COLUMNS = ["ThetaK", "X1","Y1","X2","Y2","DeltaThetaK","DeltaX1","DeltaY1","sweep_phi","Bx","By","Bz", "elapsed_time"]
    ...
    def execute(self):
        ...
        for progress_iterator, phi in enumerate(phi_points):
            ...
            while not self.magnet.check_field_polar(self.sweep_phi_field,phi,self.sweep_phi_polar, 5e-3):
                sleep(0.1)
                if self.should_stop():
                    log.info("Caught stop flag in procedure.")
                    break
...
class sagnacOpticsXportProcedure_vm(Procedure):
    ...
    def execute(self):
        ...
        start_time = time()
                ...
                # execute_process_iteration(field_points, num_progress, start_time)
            # execute_process_iteration(field_points, num_progress, start_time)
    ...
    def shutdown(self):
        ...
        while (abs(self.device.magnet.getH(0)) > 0.001):
            sleep(2)
        log.info(f"Field set to {0}T")
        if self.voltage_sweep:
            v_current = self.lockin.get_vout(1, 6)
            voltage_ramp_down = np.arange(v_current,0,-0.01)
            for v in voltage_ramp_down: 
                v = float(v)
                if np.isclose(0, v, atol=0.001):
                    v = 0
                # print(i)
                self.lockin.set_vout(1, 6, v)
                sleep(0.5)
