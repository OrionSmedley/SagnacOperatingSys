...
from sagnac.procedures import sagnacDCTiltCurrentProcedure

class sagnacDCTiltCurrentGUI(ManagedWindow):

    SWEEP_PARAM_NAMES = ['current']
    NUM_SWEEP_PARAMS = len(SWEEP_PARAM_NAMES)

    def __init__(self):
        super(sagnacDCTiltCurrentGUI, self).__init__(
            procedure_class=sagnacDCTiltCurrentProcedure,
            displays=[
                'sample_name',
                'f_eom',
                'field',
        ...
        self.setWindowTitle('PyMeasure Sagnac DCTilt Current Hysteresis Scan')
    ...
    def _setup_ui(self):
        ...
        """
        super(sagnacDCTiltCurrentGUI, self)._setup_ui()
        self.inputs.hide()
        self.run_directory = os.path.dirname(os.path.realpath(__file__))
        self.inputs = fromUi(os.path.join(self.run_directory,'custom_inputs/sagnac_gui_DCTiltCurrent.ui'))
    ...
    def make_procedure(self):
        ...
        """
        procedure = sagnacDCTiltCurrentProcedure()
        ...
        procedure.field = self.inputs.sweep_field.value()
        procedure.field_azimuth = self.inputs.sweep_field_azimuth.value()
        procedure.field_polar = self.inputs.sweep_field_polar.value()
    ...
    def queue(self):
        ...
        for procedure in procedures:
            ...
            pre = procedure.sample_name + \
                '_SagnacDCTiltCurrent_I{current:0.4f}T_A{azimuth:0.1f}_'.format(
                current=procedure.field,
