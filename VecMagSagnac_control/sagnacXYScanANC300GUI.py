...
from sagnac.procedures import XYScanANC300Procedure

#import send_gmail



class sagnacXYScanANC300GUI(ManagedWindow):

	def __init__(self):
		super(sagnacXYScanANC300GUI, self).__init__(
			procedure_class = XYScanANC300Procedure,
	...
	def _setup_ui(self):
		...
		"""
		super(sagnacXYScanANC300GUI, self)._setup_ui()
		self.inputs.hide()
		self.run_directory = os.path.dirname(os.path.realpath(__file__))
		self.inputs = fromUi(os.path.join(self.run_directory,'./custom_inputs/xy_scan_sagnac_field_gui.ui'))

	def make_procedure(self):
		"""
		Constructs a single procedure
		"""
		procedure = XYScanANC300Procedure()
	...
	def queue(self):
		...
		for procedure in procedures:
			...
			suf = '_XYScan_forwt%d'%procedure.forw_total + '_slowt%d'%procedure.slow_total + '_B%fT'%procedure.field_strength + '_A%d'%procedure.field_azimuth + '_P%d'%procedure.field_polar 
