...
class daedalusRadialVoltCalibrationProcedure(Procedure):
    """
    Procedure for calibrating the voltage to field strength relationship
    on Daedalus at some radial distance. Assumes that the center calibration has
    already ran. Only done along y axis, phi=0.
    """

    # control parameters
    calib_name = Parameter("Calibration Name")

    radial_dist = FloatParameter("Radial Distance", units='mm', default=0.)
    ...
    def startup(self):
        ...
        log.info("Setting magnet position to X: {0}, Y: {1}".format(self.centerX, self.centerY + self.radial_dist))
        self.magnet.motion_inst.x.position = self.centerX
        self.magnet.motion_inst.y.position = self.centerY + self.radial_dist
    ...
    def execute(self):
        ...
        for progress_iterator, v in enumerate(voltage_points):
            ...
            self.emit('results',
                      {
                          "V": self.magnet.volts,
                          "Xfield": self.get_Bx_zeroed,
                          "Yfield": self.get_By_zeroed,
                          "Zfield": self.get_Bz_zeroed,
...
class daedalusRadialVoltCalibrationGUI(ManagedWindow):
    def __init__(self):
        super().__init__(
            procedure_class=daedalusRadialVoltCalibrationProcedure,
            displays=[
                'calib_name',
                'voltage_start',
                'voltage_stop',
    ...
    def _setup_ui(self):
        ...
        self.inputs = fromUi(os.path.join(self.run_directory,
                                          'radialVolt_calibration_gui.ui'))

    def make_procedure(self):
        procedure = daedalusRadialVoltCalibrationProcedure()
        ...
        procedure.radial_dist = self.inputs.radial_dist.value()
    ...
    def queue(self):
        fname = unique_filename(
            ...
            dated_folder=True,
            prefix=self.inputs.calib_name.text() + '_daedalus_radialVolt_calib_',
            suffix=''
        )
...
if __name__ == "__main__":
    ...
    window = daedalusRadialVoltCalibrationGUI()
    window.show()
    sys.exit(app.exec_())
